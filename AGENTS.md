...
- Key functions:
  ...
  - PDF sticker generation uses `pdf-lib` and client-side QR PNG conversion; layout is fixed to A4 12-up (70 x 67.7 mm) with a 14.7 mm top margin, 50 mm QR centered within each cell, top title offset 5 mm, subtitle gap 4.2 mm, and equalized text-to-QR gaps derived from the legacy spacing while clamping to the available space to prevent overflow (src/components/qr/qr-code-list.tsx).
